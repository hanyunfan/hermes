#!/usr/bin/env python3
"""
Who-is-AI server — Flask + WebSocket (flask-sock)
Routes:
  GET  /                  → index.html (room lobby / game UI)
  GET  /room/<id>         → same index.html (room loaded client-side)
  WS   /ws                → real-time bidirectional events
"""
...
# ws_id → {game, player_seat, is_spectator}
conn_state: dict[str, dict] = {}


# ── helpers ──────────────────────────────────────────────────────
def broadcast(room_id: str, msg: dict, exclude: str = None):
    """Send JSON to all WS connections in a room."""
    data = json.dumps(msg)
    for wid, state in list(conn_state.items()):
        if wid == exclude:
            continue
        g = state.get("game")
        if g and g.room_id == room_id:
            try:
                clients[wid].send(data)
            except Exception:
                pass
...
def send_to(ws_id: str, msg: dict):
    if ws_id in clients:
        clients[wid].send(json.dumps(msg))


clients: dict[str, object] = {}  # ws_id → flask-sock Connection
...
async def chat_timer(game: Game, ai_mgr: AIManager):
    ...
    # Generate AI messages for this round before voting starts
    await generate_ai_round(game, ai_mgr)

    # Broadcast chat locked + voting starts
...
async def generate_ai_round(game: Game, ai_mgr: AIManager):
    """Let each AI player decide if they want to speak this round."""
    for seat, ai in ai_mgr.players.items():
        ...
        if text:
            # simulate typing delay then inject
            await asyncio.sleep(ai.typing_delay() + random.uniform(0.5, 2.0))
...
# ── vote timeout ─────────────────────────────────────────────────
def voting_timeout(game: Game):
    """30s after voting starts, auto-resolve."""
    time.sleep(30)
    if game.phase != GamePhase.VOTING:
        return
    # Force resolve with current votes (missing voters skip)
    game._resolve_vote()


# ── AI game manager map ────────────────────────────────────────────
ai_managers: dict[str, AIManager] = {}  # room_id → AIManager
    ...
    try:
        while True:
            ...
            if msg_type == "create_room":
                ...
                if not game:
                    ws.send(json.dumps({"type": "error", "reason": "房间不存在"}))
                    continue
                if game.password and game.password != password:
                    ws.send(json.dumps({"type": "error", "reason": "房间密码错误"}))
                    continue
                if game.phase not in (GamePhase.WAITING,):
                    ws.send(json.dumps({"type": "error", "reason": "游戏已开始，无法加入"}))
                    continue
                ...
                if existing_seat:
                    ...
                    if seat is None:
                        ...
                        state["player_seat"] = None
                        ws.send(json.dumps({"type": "spectating", "room_id": room_id}))
                        ws.send(json.dumps({
                            "type": "room_state",
                    ...
                    player = game.add_player(seat, ws_id)
                ...
                if game.ready_to_start():
                    ...
                    broadcast(game.room_id, {
                        "type": "game_start",
                        "message": f"游戏开始！{game.round_num}号房间，共{Game.MODE_SEATS[game.mode]}人",
                    }, exclude=ws_id)
                    ws.send(json.dumps({
                        "type": "game_start",
                        "message": "游戏开始！",
            ...
            # ── vote ─────────────────────────────────────────────────
                ...
                if complete:
                    ...
                    eliminated_seat = next(
                        (s for s, p in game_obj.seats.items() if not p.is_alive and
            ...
            # ── get state ───────────────────────────────────────────
                ...
                game = state.get("game")
                if game:
                    ws.send(json.dumps({"type": "room_state", **game.public_state(ws_id)}))
        ...
        if game:
            ...
            if state.get("is_spectator"):
                ...
                # Mark player disconnected but don't eliminate
                p = game.seats[seat]
                # Don't remove seat — just mark ws_id None
                # They can reconnect
