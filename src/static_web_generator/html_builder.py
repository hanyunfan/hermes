...
function displayAllMonths() {{
  ...
  // Show Aug of the next school year through its May end.
  // School years: Aug of start year through May of following year.
  // Data covers: 2025-2026 (Aug 2025-May 2026) and 2026-2027 (Aug 2026-May 2027).
  // Always show the UPCOMING school year, capped at May 2027.
  let displayStart, displayEnd;
  if (nowMonth >= 7) {{
    // In Aug-Dec: next SY started this Aug (nowYear), ends May nowYear+1
    // Show Aug nowYear+1 through May nowYear+2 (cap at May 2027)
    displayStart = new Date(nowYear + 1, 7, 1);
    displayEnd   = new Date(nowYear + 2, 5, 1);  // June 1 = end of May
  }} else {{
    // In Jan-Jul: next SY starts Aug this year, ends May next year
    displayStart = new Date(nowYear, 7, 1);
    displayEnd   = new Date(nowYear + 1, 5, 1);  // June 1 = end of May
  }}
  // Cap at June 1 2027 so we show through May 2027 (last month with school data)
  const DATA_END = new Date(2027, 5, 1);  // June 1 2027 = end of May 2027
